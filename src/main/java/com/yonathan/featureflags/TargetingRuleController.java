...
public class TargetingRuleController {
	...
	@PostMapping @ResponseStatus(HttpStatus.CREATED) public TargetingRule create(@PathVariable Environment environment, @PathVariable String flagKey, @Valid @RequestBody CreateTargetingRuleRequest request) { return service.create(environment, flagKey, request.userId(), request.priority()); }
}
